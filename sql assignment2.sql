...
-- Giving alias name for age and salary in employee table.alter
...
SELECT  sum(salary) fin_dept_salary FROM employees 
WHERE department_id = 7;

-- Find the minimum age among all employees.

SELECT min(age) Minimu_age FROM employees;

-- List the maximum salary for each location.

SELECT l.location_name, max(salary) Max_salary FROM location l 
...
SELECT l.location_name, avg(age) avg_age FROM location l 
LEFT JOIN employees e
ON l.location_id = e.location_id
WHERE gender = 'F'
...
SELECT department_name, count(e.employee_id) total_no_emp FROM departments d
LEFT JOIN employees e
...
SELECT employee_name, salary, RANK () OVER(ORDER BY salary DESC) EMP_RANK
FROM employees;
